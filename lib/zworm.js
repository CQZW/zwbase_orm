...
class ZWORM
{
     ...
     * @param {[]} upprops 要更新的属性列表,如果不是插入,就更新这些字段,不指定就全部
     * @param {*} [opt=null]
     * @returns{int} -1:失败,0更新成功,1插入成功(添加id(Object))
     * @memberof ZWORM
     */
    ...
    /**
     * 通过这个对象的属性作为条件更新数据
     * @param {ZWORM|Object} 要更新的数据,原生对象或者orm对象 如果=null,默认是this
     * @param {*} [opt=null] 
     ...
     * @memberof ZWORM
     */
    async updateMany( u_doc = null , opt = null ,bone = false )
    {
        let q = this._convertQuery();
        //update 操作的查询语句应该要判断下,万一更新了全部数据!!!!!!
     ...
     * @param {Array} upprops 如果匹配到数据,要更新的属性列表;(如果没有就所有属性全新插入)
     * @param {*} [opt=null]
