...
class ZWORM
{
     ...
     * @param {boolean} forcerelock 是否强制重新锁定所有字段,否则如果已经锁定过直接返回
     * @memberof ZWORM
     */
     ...
     * 关联查询
     *
     * @param {*} joinfrom 被关联的表或者对象
     * @param {*} joinfromid 被关联表里面的字段名或者属性名
     * @param {*} joinid 用于关联相等的字段名或者属性名
     * @param {*} joinas 关联后的输出名字
     * @returns
     * @memberof ZWORM
     */
     ...
     * 预处理所有属性作为查询条件数据
     * 全部合并到一个查询对象
     * @param {*} [keys=null],指定处理那些属性,如果不知道就是锁定的属性
     * @returns
     * @memberof ZWORM
     */
     ...
     * @param {*} [opt=null]
     * @returns{int} -1:失败,0更新成功,1插入成功
     * @memberof ZWORM
     */
    async dumpThisById( opt = null )
    {
        try
        {
            return Promise.resolve( await this.insertThis( opt ) );
        }
        catch( e )
        {
            if( e.code != 11000 ) return Promise.resolve(false);
        }
        //走到这里,说明是插入重复了,需要更新
        this[ this.getPropIdName() ] = ZWQuery.Query('==',this[ this.getPropIdName() ]);
        return this.updateThis( this );
    }
    ...
    {
        ////https://my.oschina.net/killnull/blog/748928
        let id = new OBJECTID();
