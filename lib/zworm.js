...
class ZWORM
{
    ...
    /**
     * 通过这个对象的属性作为条件更新数据,不会自动插入新数据,如果u_doc=null,属性值不是Query类型的就是要更新的数据
    ...
    /**
     * 原子更新,不会自动插入,如果u_doc=null,属性值不是Query类型的就是要更新的数据
     * 成功会把最新数据映射到this
    ...
    enCodeForDump()
    {
        return this.enCodeObj( this , 'this' , true );
    }
    enCodeObj( obj , path ,needzworm )
    {
        let r = {};
        ...
        for( let one of keys )
        {
            let nowpath = path + '.' + one;
            if( one.startsWith('_') ) continue;
            let v = obj[one];
            if( !v ) continue;
            if( typeof v == 'symbol' ) 
            {
                console.warn('not support symbol type');
                continue;
            }
            if( typeof v == 'function' ) continue;

            //如果不是对象类型,复制就够了,不需要标记类型了
            if( typeof v != 'object' )
            {
                r[one] = v;
                continue;
            ...
            {
                //如果不需要zworm类型的数据
                if( !needzworm ) continue;
                let _x = v.enCodeForDump();
                r[one] = _x.r;
                r[ one + '_cls_zworm' ] = _x._cls_zworm;
                continue;
            }
            //如果是数组,把内容处理了就行了,数组类型不变
            if( Array.isArray(v) )
            {
                let a = [];
                for( let i = 0; i < v.length ;i ++ )
                {
                    a.push( this.enCodeObj( v[i] , nowpath + '[' + i + ']' ) );
                }
                r[one] = a;
                continue;
            }
            
            let _tcls = Object.getPrototypeOf(v).constructor.name;
            if( _tcls == 'Object' )
            {//如果是对象就需要继续拆分
                let _x  = this.enCodeObj( v );
                r[one]  = _x.r;
                r[one+'_cls_zworm']  = _x._cls_zworm;
                continue;
            }
            //剩下的都是全局的系统类型了
            let _x = this.encodeGlobalObj( v );
            r[one] = _x.r;
            r[one + '_cls_zworm'] = _x._cls_zworm;
        }
        return {r:r,_cls_zworm:this.getClsNameByObj( obj, path )};
    }
    /**
     * 将编码的数据反编到自己对应的属性,
     *
    ...
    decodeObj( obj ,cls )
    {
        ...
        if( Array.isArray( obj ) )
        {
            ...
            for( let i of obj )
            {
                r.push( this.decodeObj( i.r , i._cls_zworm  ) );
            }
            return r;
        }
        //如果没有类名标签,直接返回了
        ...
        r = this.getObjByClsName( cls  , obj );
        if( !r ) r = this.getGlobalObjByClsName( cls ,obj );
        return r;
    }
    /**
     * 实例化数据先走 getObjByClsName ,如果返回Null,到这里,
     * @param {string} clsname
     * @param {*} v
     * @returns 返回全局的类型实例化
     * @memberof ZWORM
     */
    getGlobalObjByClsName( clsname , v )
    {
        if( clsname == 'Date' && typeof v =='string' ) return new Date( v );
        //更多的系统,全局类型处理,加到这里,子类继承即可
        
        return v;
    }
    encodeGlobalObj( v )
    {
        return {r:v,_cls_zworm:Object.getPrototypeOf(v).constructor.name};
    }
    /**
     ...
     * @param {*} v 数据
     * @returns {} 如果没找到对应的类,返回null 那么 最终会落到 getGlobalObjByClsName 负责最终系统类型的处理
     * @memberof ZWORM
     */
    getObjByClsName( clsname , v )
    {
        ...
        if( clsname.endsWith('_zworm') )
        {
            ...
            if( !_cls )
            {
                console.warn('not find spec cls:',_clsname,' require it in you module or Global cls');
                return null;
            }
            /**
             * @type ZWORM
            ...
            return _v.decodeFromDump( {r:v} );
        }
        //其他情况先不管,直接返回Null,给子类处理,或者系统类型处理
