...
class ZWORM
{
     ...
     * @memberof ZWORM
     */
    async updateById( id , opt = null )
    {
        let q = new this._cls();
        q[ q.getPropIdName() ] = ZWQuery.Query('==',id);
        return await this.updateThisBy( q,null,opt );
    }

    /**
     * 如果 u_doc = null,他属性作为更新的数据,否则忽略属性,直接更新u_doc
     * @param {ZWORM|Object} query 
     * @param {Object} [u_doc=null] 要更新的自定义文档
     * @param {*} [opt=null] 
     * @returns{int} 返回更新了多少条数据
     * @memberof ZWORM
     */
    async updateThisBy( query ,u_doc = null ,opt = null )
    {
        return this.updateManyBy( query,u_doc,opt,true);
    }

    /**
     * 更新多个数据
     * 和updateThisBy唯一区别就是更新多个或者一个
     * @param {*} query
     * @param {*} [u_doc=null]
     * @param {*} [opt=null]
     ...
     * @memberof ZWORM
     */
    async updateManyBy( query ,u_doc = null, opt = null ,bone = false )
    {
        let q = query;
